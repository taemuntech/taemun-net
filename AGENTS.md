...
1. **규칙 정본은 [`docs/PORTFOLIO_FACTORY.md`](docs/PORTFOLIO_FACTORY.md) §0 (1~13번)** — 사이트 이름은 「태문넷」·카드 JSON·`(demos)` 폴더·실존 이름 금지·조회되는 번호 금지·가짜 접수 금지·원본 화질 이미지·`<SampleFooterNote />`·`DevicePreviewFrame`·본문 최대 1280px·기술 스택엔 실제로 쓴 기술만·제작 기간은 「예상」.
