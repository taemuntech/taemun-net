          ...
          <div className="flex flex-wrap items-center gap-2 mt-6 lg:mt-0">
            {(['All', 'Retail', 'F&B', 'Lounge', 'Showroom'] as const).map((cat) => (
              <button
                ...
                className={`px-4 py-2 rounded-sm text-xs font-mono tracking-wider transition-all ${
                  filter === cat
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {filteredProjects.map((project) => (
            <div
              ...
              className="group cursor-pointer rounded-sm bg-stone-900/40 border border-white/10 hover:border-amber-400/50 overflow-hidden transition-all duration-300 flex flex-col justify-between"
            >
              ...
              <div className="p-6 space-y-4">
                ...
                {/* Meta details */}
                <div className="grid grid-cols-3 gap-2 text-[11px] font-mono py-3 border-y border-white/5 text-stone-400">
                  <div>
                ...
                <div className="space-y-1.5 pt-1">
                  {project.features.slice(0, 2).map((feat, i) => (
                    <div key={i} className="flex items-center text-xs text-stone-300">
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-400/80 mr-2 shrink-0" />
                ...
                <div className="pt-3 flex items-center justify-between text-xs font-mono text-amber-400 group-hover:underline">
                  <span>프로젝트 상세 스토리 및 고객 후기 보기</span>
                  <span>→</span>
                </div>
              </div>
            </div>
