...
import { X, Play, Pause, Download, ShieldCheck, CheckCircle2, FileText, Atom, Layers, Sparkles } from 'lucide-react';
...
export const MoAVideoModal: React.FC<MoAModalProps> = ({ isOpen, onClose }) => {
  ...
  const steps = [
    ...
    { title: "04. 26S 프로테아좀 완벽 분해 및 재순환", desc: "암세포 내 프로테아좀이 KRAS 표적을 아미노산 단위로 완전 소멸시키며, PROTAC은 분리되어 차기 표적을 연속 분해합니다." }
  ];
...
export const ProtocolModal: React.FC<ProtocolModalProps> = ({ item, onClose }) => {
  ...
  const handleDownload = () => {
    setDownloaded(true);
    setTimeout(() => {
      alert(`[${item.code}] 임상시험 프로토콜 요약본(PDF) 열람용 전자 서명이 확인되었습니다.`);
    }, 300);
  };
      ...
      <div className="bg-white max-w-xl w-full rounded-2xl border border-[#c4c5d5] shadow-2xl p-6 relative">
        ...
        <div className="mt-6 flex justify-end space-x-3 pt-3 border-t border-[#c4c5d5]/30">
          ...
          >
            ...
            <span>{downloaded ? '다운로드 완료됨' : '보안 PDF 다운로드'}</span>
          </button>
        </div>
      </div>
    </div>
        ...
        <div className="mt-6 flex justify-end space-x-3 pt-3 border-t border-[#c4c5d5]/30">
          ...
          <button
            onClick={() => {
              setDownloaded(true);
              setTimeout(() => {
                alert(`[${publication.journal}] 논문 브리핑 리프린트가 성공적으로 다운로드되었습니다.`);
              }, 300);
            }}
            className="px-5 py-2 bg-[#1e40af] text-white rounded-lg text-[13px] font-semibold hover:bg-[#00288e] transition flex items-center space-x-1.5 shadow-sm cursor-pointer"
          >
            {downloaded ? <CheckCircle2 className="w-4 h-4" /> : <Download className="w-4 h-4" />}
            <span>{downloaded ? '다운로드 완료됨' : '논문 전문 PDF 다운로드'}</span>
          </button>
        </div>
      </div>
    </div>
