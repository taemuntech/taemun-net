        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-12">
          <div>
            ...
            <p className="text-zinc-400 text-sm mt-2 font-light">
              존을 선택하고 이미지 속 핀을 클릭하여, 협업과 초집중을 넘나드는 시공 디테일을 확인해 보세요.
            </p>
          </div>
          ...
          <div className="flex items-center gap-1.5 mt-6 lg:mt-0 bg-zinc-900 p-1.5 rounded-xl border border-zinc-800 overflow-x-auto [scrollbar-width:none]">
            {WORKSPACE_ZONES.map((zone) => (
              <button
                ...
                className={`px-3.5 py-2 rounded-lg text-xs font-mono font-semibold tracking-wider transition-all whitespace-nowrap ${
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="relative aspect-[16/10] w-full rounded-2xl overflow-hidden border border-white/10 bg-zinc-950 group">
              ...
              {currentZone.hotspots.map((spot, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Popover */}
                    {isActive && (
                      <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-3 w-64 p-3.5 rounded-xl bg-zinc-950/95 border border-cyan-400/40 backdrop-blur-md shadow-2xl text-left z-30 animate-in fade-in zoom-in-95 duration-150">
              ...
              <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between pointer-events-none">
                ...
                <div className="text-[11px] text-zinc-400 bg-black/70 px-2.5 py-1 rounded-lg border border-white/10">
                  핀을 누르면 시공 공법이 안내됩니다
                </div>
              </div>
            </div>
              ...
              {currentZone.hotspots.map((spot, idx) => (
                ...
                >
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <span className="text-xs font-semibold truncate text-zinc-200">{spot.title}</span>
                  </div>
                  <p className="text-[10px] text-zinc-400 truncate font-light">{spot.desc}</p>
                </button>
              ))}
            </div>
          </div>
