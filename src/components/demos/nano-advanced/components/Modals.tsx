...
import { X, CheckCircle2, Download, Printer, Layers, Cpu, ShieldCheck } from 'lucide-react';
...
export const WhitepaperModal: React.FC<WhitepaperModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleDownload = (id: number, title: string) => {
    ...
    setTimeout(() => {
      ...
      alert(`[다운로드 완료] "${title}" 사양서가 정상적으로 전달되었습니다.`);
    }, 800);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-2xl w-full border border-[#c4c5d5]/50 shadow-2xl overflow-hidden flex flex-col">
          ...
          {papers.map((p) => (
            ...
            >
              ...
              >
                ...
                <span>{downloadingId === p.id ? '다운로드 중...' : 'PDF 다운로드'}</span>
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="px-6 py-4 border-t border-[#c4c5d5]/30 bg-[#f8f9ff] flex justify-end">
          <button
            onClick={onClose}
            className="bg-white border border-[#c4c5d5]/70 text-[#0b1c30] px-4 py-2 rounded-lg text-xs font-semibold hover:bg-gray-50"
          >
            닫기
          </button>
        </div>
...
// 4. Consultation Submission Success Modal
...
export const SuccessModal: React.FC<SuccessModalProps> = ({ isOpen, onClose, data }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-lg w-full border border-[#c4c5d5]/50 shadow-2xl overflow-hidden p-6 text-center">
        <div className="w-14 h-14 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-4">
          <CheckCircle2 className="w-8 h-8" />
        </div>

        <h3 className="text-xl font-bold text-[#0b1c30] mb-2">
          기술 미팅 및 샘플 신청이 완료되었습니다
        </h3>
        <p className="text-sm text-[#444653] mb-6 leading-relaxed">
          <strong>{data.company}</strong> ({data.name} 님)의 신청 내역이 NANO ADVANCED 첨단 패키징 R&amp;D
          센터로 전달되었습니다. 전담 수석 엔지니어가 24시간 이내 사전 NDA 검토본과 함께 공식
          연락드립니다.
        </p>

        <div className="bg-[#eff4ff] p-4 rounded-xl border border-[#c4c5d5]/40 text-left text-xs font-mono space-y-1.5 mb-6 text-[#444653]">
          <div>
            <span className="text-[#757684]">접수 번호:</span> #NA-REQ-2026-
            {Math.floor(1000 + Math.random() * 9000)}
          </div>
          <div>
            <span className="text-[#757684]">목표 분야:</span> {data.sector}
          </div>
          <div>
            <span className="text-[#757684]">희망 솔루션:</span> {data.solutions.join(', ')}
          </div>
          <div>
            <span className="text-[#757684]">담당 이메일:</span> {data.email}
          </div>
        </div>

        <button
          onClick={onClose}
          className="w-full bg-[#00288e] hover:bg-[#1e40af] text-white py-3 rounded-lg text-sm font-semibold transition-colors"
        >
          확인
        </button>
      </div>
