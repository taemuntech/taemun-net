...
export default function VerdeGourmetApp({ isEmbed = false }: { isEmbed?: boolean }) {
  ...
  const [currentAddress, setCurrentAddress] = useState<string>('서울 용산구 한남대로 98 (베르데빌딩)');
  ...
  const handleAddRecipeBundle = (selectedItems: RecipeIngredient[], bundlePrice: number) => {
    ...
    const bundleProduct: Product = {
      ...
      name: `미슐랭 셰프 레시피 패키지 (${selectedItems.length}종 재료)`,
      origin: '미슐랭 셰프 권우성 감수',
  ...
  const handleCheckout = () => {
    ...
    showToast(
      '샛별배송 주문이 완료되었습니다!',
      '내일 오전 07:00 전 고객님의 문 앞으로 신선하게 배송됩니다.'
    );
    setCart([]);
  };
