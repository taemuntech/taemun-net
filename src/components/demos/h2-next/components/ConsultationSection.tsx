...
import { Send, Lock, CheckCircle } from './Icons';

interface ConsultationSectionProps {
  onSubmitSuccess: (data: { company: string; name: string; type: string }) => void;
}

export const ConsultationSection: React.FC<ConsultationSectionProps> = ({ onSubmitSuccess }) => {
          ...
          <form className="space-y-8" id="consultationForm" onSubmit={handleSubmit}>
            ...
            <div>
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 bg-[#00685f] hover:bg-[#008378] text-white text-base font-semibold py-4 rounded-xl shadow-md hover:shadow-lg transition-all duration-200 active:scale-95 cursor-pointer disabled:opacity-50"
              >
                <span>
                  {isSubmitting ? '신청서 암호화 전송 중...' : '전문 에너지 컨설턴트 1:1 미팅 신청'}
                </span>
              ...
              <div className="flex flex-wrap items-center justify-center gap-3 text-[#6d7a77] font-mono text-[11px] mt-3">
                <span className="flex items-center gap-1">
                  <Lock className="w-3.5 h-3.5" /> 256-bit SSL 엔드투엔드 암호화 전송
                </span>
                <span>•</span>
                <span>영업일 기준 24시간 이내 수석 컨설턴트 유선 배정</span>
              </div>
            </div>
          </form>
        </div>
