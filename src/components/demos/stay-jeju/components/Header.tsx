'use client';
import React, { useState } from 'react';
import { STAY_BRAND } from '../data/stayData';

interface HeaderProps {
  onNavigate: (sectionId: string) => void;
  onOpenConsultation: () => void;
}

export const Header: React.FC<HeaderProps> = ({ onNavigate, onOpenConsultation }) => {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  const handleNavClick = (sectionId: string) => {
    onNavigate(sectionId);
    setMobileMenuOpen(false);
  };

  return (
    <header className="sticky top-[var(--sample-bar-h,0px)] z-40 w-full bg-[#18191d]/90 backdrop-blur-md border-b border-stone-800 text-stone-200 transition-all">
      <div className="max-w-7xl mx-auto px-4 lg:px-8 h-20 flex items-center justify-between">
        {/* Logo */}
        <button
          type="button"
          className="flex items-center gap-3 cursor-pointer text-left"
          onClick={() => handleNavClick('hero')}
          aria-label="소소재 제주 — 맨 위로"
        >
          <span className="w-10 h-10 shrink-0 rounded-sm bg-stone-100 text-stone-900 flex items-center justify-center font-serif font-bold text-lg tracking-tighter">
            素
          </span>
          <span className="block">
            <span className="flex items-center gap-2">
              <span className="font-serif tracking-widest text-lg font-bold text-stone-100">
                {STAY_BRAND.name}
              </span>
              <span className="hidden lg:inline-block text-[10px] uppercase font-mono tracking-widest px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-300 border border-amber-500/20">
                Boutique Stay
              </span>
            </span>
            <span className="block text-[11px] text-stone-400 font-sans tracking-wide">
              제주 애월 독채 프리미엄 스테이 디자인
            </span>
          </span>
        </button>

        {/* Desktop Navigation */}
        <nav className="hidden lg:flex items-center gap-8 text-sm font-medium text-stone-300">
          <button
            onClick={() => handleNavClick('timemood')}
            className="hover:text-amber-300 transition-colors tracking-wider"
          >
            시간의 빛 (무드 전환)
          </button>
          <button
            onClick={() => handleNavClick('zones')}
            className="hover:text-amber-300 transition-colors tracking-wider"
          >
            공간 디테일 투어
          </button>
          <button
            onClick={() => handleNavClick('materials')}
            className="hover:text-amber-300 transition-colors tracking-wider"
          >
            제주 물성 아카이브
          </button>
          <button
            onClick={() => handleNavClick('philosophy')}
            className="hover:text-amber-300 transition-colors tracking-wider"
          >
            공간 철학
          </button>
        </nav>

        {/* Action Button & Mobile Toggle */}
        <div className="flex items-center gap-4">
          <button
            onClick={onOpenConsultation}
            className="hidden lg:inline-flex items-center justify-center px-5 py-2.5 rounded-sm bg-amber-600/90 hover:bg-amber-500 text-stone-950 font-semibold text-xs tracking-widest uppercase transition-all shadow-md"
          >
            스테이 시공 & 투어 문의
          </button>

          <button
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className="lg:hidden p-2.5 -mr-1 text-stone-300 hover:text-white focus:outline-none"
            aria-label={mobileMenuOpen ? '메뉴 닫기' : '메뉴 열기'}
            aria-expanded={mobileMenuOpen}
          >
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              {mobileMenuOpen ? (
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              ) : (
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" />
              )}
            </svg>
          </button>
        </div>
      </div>

      {/* Mobile Navigation Dropdown */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-[#18191d] border-b border-stone-800 px-6 py-6 space-y-4 animate-in fade-in duration-200">
          <div className="flex flex-col space-y-3 text-sm text-stone-300 font-medium">
            <button
              onClick={() => handleNavClick('timemood')}
              className="text-left py-2 hover:text-amber-300 transition-colors border-b border-white/5"
            >
              시간의 빛 · 무드 전환 (Time & Lighting)
            </button>
            <button
              onClick={() => handleNavClick('zones')}
              className="text-left py-2 hover:text-amber-300 transition-colors border-b border-white/5"
            >
              공간 디테일 투어 (Spatial Tour)
            </button>
            <button
              onClick={() => handleNavClick('materials')}
              className="text-left py-2 hover:text-amber-300 transition-colors border-b border-white/5"
            >
              제주 물성 아카이브 (Jeju Materials)
            </button>
            <button
              onClick={() => handleNavClick('philosophy')}
              className="text-left py-2 hover:text-amber-300 transition-colors border-b border-white/5"
            >
              공간 철학 (Philosophy)
            </button>
          </div>

          <div className="pt-2">
            <button
              onClick={() => {
                setMobileMenuOpen(false);
                onOpenConsultation();
              }}
              className="w-full py-3 text-center rounded-sm bg-amber-600 hover:bg-amber-500 text-stone-950 font-bold text-xs tracking-widest uppercase"
            >
              스테이 공간 시공 상담 신청
            </button>
          </div>
        </div>
      )}
    </header>
  );
};
