...
export const DiagnosticSuite: React.FC<DiagnosticSuiteProps> = ({ language }) => {
  const devices = [
    {
      name: '3D 각막 단층 분석기',
      sub: '회전 샤임플러그 각막 토포그래피',
      metric: '138,000 포인트 입체 스캔 (예시 사양)',
      icon: 'blur_on',
      desc: '회전형 정밀 카메라로 각막 전면뿐 아니라 후면 곡률, 전방 깊이, 잠재적 원추각막 징후까지 미세 단위로 입체 촬영합니다.',
    },
    {
      name: '고해상도 망막 OCT',
      sub: '망막 · 시신경 광간섭 단층 촬영',
      metric: '초당 100,000회 A-스캔 (예시 사양)',
      icon: 'radar',
      desc: '망막 중심 황반부의 미세 병변과 녹내장 조기 확인을 위한 시신경 섬유층 두께를, 눈에 닿지 않는 광간섭 방식으로 촬영합니다.',
    },
    {
      name: '각막 생체역학 분석기',
      sub: '각막 탄성도 · 변형 반응 분석',
      metric: '초고속 4,330 fps 카메라 (예시 사양)',
      icon: 'air',
    ...
    },
    {
      name: '광학식 안축장 계측기',
      sub: '백내장 인공수정체 도수 계산',
      metric: '광학식 비접촉 계측 (예시 사양)',
      icon: 'straighten',
      desc: '광간섭 방식으로 각막 중심부터 망막 중심와까지의 안구 길이를 재어, 백내장 수술에 넣을 인공수정체의 도수를 계산합니다.',
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="max-w-3xl mb-10 lg:mb-12 break-keep">
          ...
          <p className="font-body-lg text-[14px] lg:text-[17px] text-on-surface-variant mt-2 leading-relaxed">
            {language === 'KR'
              ? '수술 전 놓치기 쉬운 해부학적 위험 요소를 걸러내기 위해, 정밀 진단 장비 4종의 결과를 교차 확인합니다. 장비 명칭과 사양은 예시입니다.'
              : 'Four diagnostic instruments are cross-checked before surgery to screen anatomical risk factors. Device names and specifications shown are examples.'}
          </p>
        </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="font-body-md text-[14px] text-primary-fixed/90 mt-2 max-w-3xl leading-relaxed">
                잔여 각막 두께 최소 350㎛ 엄격 준수 (규제기관(예시) 기준 250㎛보다 100㎛ 더 안전한 마진 확보). 유전성 아벨리노 각막이상증 DNA 5종 정밀 검사를 전원 의무 시행하며, 3차 크로스 체킹을 통과하지 못하면 수술 불가로 판정하고 비수술 대안을 안내합니다.
              </p>
            </div>
          </div>
