...
export const PrivateSuiteTour: React.FC<PrivateSuiteTourProps> = ({ onOpenBooking }) => {
  ...
  const amenities = [
    {
      title: '다이슨 에어랩 & 슈퍼소닉 헤어스타일링',
      desc: '시술 후 프라이빗 파우더룸에서 흐트러진 헤어를 완벽하게 리셋할 수 있도록 다이슨 최신 스타일링 툴이 비치되어 있습니다.',
    },
    {
      title: '라 메르(La Mer) 기초 진정 스킨케어',
      desc: '민감해진 피부를 부드럽게 감싸주는 라 메르의 미라클 브로스 성분 스킨케어 라인이 1인 1실마다 상시 구비됩니다.',
    },
    {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center mb-12">
          <div className="lg:col-span-6">
            ...
            <p className="text-sm lg:text-base text-[#424845] leading-relaxed mb-6">
              더 노블 청담은 오픈형 베드나 공용 대기실을 운영하지 않습니다. 내원 순간부터 상담, 시술, 진정 케어, 파우더룸까지 단 한 번의 이동 없이 1인 전용 독립 스위트에서 최상의 안락함을 제공합니다.
            </p>
              ...
              {amenities.map((item, idx) => (
                ...
                >
                  <div className="flex items-center gap-2 font-semibold">
                    <Check className={`w-3.5 h-3.5 ${activeAmenityIndex === idx ? 'text-[#745a2a]' : 'text-[#c1c8c4]'}`} />
                    <span>{item.title}</span>
                  </div>
                  {activeAmenityIndex === idx && (
                    <p className="text-[#424845] mt-1 pl-5.5 leading-relaxed">{item.desc}</p>
                  )}
                </div>
              ))}
            </div>
            ...
            <div className="relative rounded-xl overflow-hidden shadow-2xl h-80 lg:h-96 border border-[#ffffff]/60 group">
              ...
              <div className="absolute bottom-6 left-6 right-6 text-[#ffffff]">
                ...
                <div className="font-serif text-lg lg:text-xl text-[#ffffff] font-medium">
                  전 객실 다이슨 에어랩 & 라 메르 어메니티
                </div>
                <p className="text-xs text-[#ffffff]/80 mt-1">
                  타인과 마주치지 않는 1:1 완벽 독립 룸에서 상담부터 시술 마무리까지 진행됩니다.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="p-6 rounded-xl bg-[#ffffff] shadow-sm border border-[#e4e2df]">
            ...
            <p className="text-xs lg:text-sm text-[#424845] leading-relaxed">
              다른 고객과 마주치지 않는 1인 1실 독립 복도 동선과 특수 차음 설계를 적용하여 외부의 소음과 시선으로부터 완전히 차단된 사생활을 엄격히 지향합니다.
            </p>
          </div>
          ...
          <div className="p-6 rounded-xl bg-[#ffffff] shadow-sm border border-[#e4e2df]">
            ...
            <p className="text-xs lg:text-sm text-[#424845] leading-relaxed">
              시술 후 헝클어진 머리와 피부결을 섬세하게 정돈하실 수 있도록 다이슨 헤어스타일링 기기, 라 메르 기초 스킨케어, 1회용 멸균 비품을 개별 완비했습니다.
            </p>
          </div>
          ...
          <div className="p-6 rounded-xl bg-[#ffffff] shadow-sm border border-[#e4e2df]">
            ...
            <p className="text-xs lg:text-sm text-[#424845] leading-relaxed">
              0.3㎛ 초미세먼지와 바이러스를 99.995% 차단하는 고성능 HEPA-14 음·양압 필터링 시스템을 각 룸마다 독립 가동하여 완벽한 청정 멸균 공기를 유지합니다.
            </p>
          </div>
        </div>
      </div>
