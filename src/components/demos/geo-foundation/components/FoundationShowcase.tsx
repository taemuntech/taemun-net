...
import { FOUNDATION_PROJECTS } from '../data/geoData';
import { FoundationProject } from '../types';
...
export default function FoundationShowcase() {
  ...
  const filteredProjects = activeFilter === 'all'
    ? FOUNDATION_PROJECTS
    : FOUNDATION_PROJECTS.filter((p) => p.category === activeFilter);
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-16 gap-6">
          <div>
            ...
            <p className="text-slate-400 text-base mt-2 max-w-2xl">
              지하철 초인접 대심도 굴착부터 해안 초고압 차수 그라우팅까지, 지오파운데이션의 정밀 토목 기술이 집약된 프로젝트입니다(예시).
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={() => setActiveFilter('deep-excavation')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-colors ${
                activeFilter === 'deep-excavation'
                  ? 'bg-cyan-500 text-slate-950'
                  : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'
              }`}
            >
            ...
            <button
              type="button"
              onClick={() => setActiveFilter('d-wall')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-colors ${
                activeFilter === 'd-wall'
                  ? 'bg-cyan-500 text-slate-950'
                  : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'
            ...
            <button
              type="button"
              onClick={() => setActiveFilter('rcd-pile')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-colors ${
                activeFilter === 'rcd-pile'
                  ? 'bg-cyan-500 text-slate-950'
                  : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'
              }`}
            >
            ...
            <button
              type="button"
              onClick={() => setActiveFilter('grouting')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-colors ${
                activeFilter === 'grouting'
                  ? 'bg-cyan-500 text-slate-950'
                  : 'bg-slate-900 text-slate-400 hover:text-white border border-slate-800'
              }`}
            >
              ...
              <div className="p-6 flex-1 flex flex-col justify-between">
                ...
                <div className="space-y-2 mb-6 border-t border-slate-800 pt-4 text-xs">
                  <div className="flex justify-between py-1">
                    <span className="text-slate-500">굴착 및 시공 심도</span>
                    <span className="text-slate-200 font-bold">{project.depth}</span>
                  </div>
                  <div className="flex justify-between py-1">
                    <span className="text-slate-500">적용 벽체 공법</span>
                    <span className="text-slate-200 font-bold">{project.wallType}</span>
                  </div>
                  <div className="flex justify-between py-1">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 shadow-2xl">
            ...
            <div className="space-y-6 text-sm">
              ...
              <div>
                <h4 className="text-xs font-bold text-cyan-400 uppercase mb-3">핵심 엔지니어링 기술 사양</h4>
