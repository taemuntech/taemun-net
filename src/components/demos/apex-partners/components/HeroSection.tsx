...
export default function HeroSection({ onOpenVdr }: HeroSectionProps) {
  // 배너는 외부 호스트 이미지라 주소가 만료되면 깨진 아이콘이 남는다 — 그때는 결에 맞는 배경으로 대체한다.
  const [bannerFailed, setBannerFailed] = useState(false);

  return (
    <section className="relative pt-8 lg:pt-16 pb-20 overflow-hidden scroll-mt-24" id="hero">
...
가상 브랜드 샘플 · 실제 운용사가 아닙니다 · 화면의 수치는 예시
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end mb-12">
          <div className="lg:col-span-8">
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif-display text-[#dee2ef] tracking-tight leading-[1.1] mb-4">
              Engineering Enduring Value <br />
              <span className="gold-shimmer italic font-normal">Across Generations.</span>
            </h1>
            <p className="font-serif-display text-lg md:text-xl text-[#d0c5af] font-normal tracking-wide">
          ...
          <div className="lg:col-span-4 flex flex-col justify-end gap-6">
            <p className="text-sm text-[#d0c5af]/90 border-l border-[#f2ca50]/40 pl-4 leading-relaxed">
              글로벌 기관투자자(LP)와 함께 바이아웃, 딥테크 그로쓰 에쿼티, 크로스보더 전략으로 초과수익(Alpha)을 추구하는 운용사 설정입니다.
            </p>
            <div className="flex flex-col sm:flex-row gap-3">
              <a
                href="#portfolio"
                className="inline-flex items-center justify-center gap-2 px-5 min-h-12 bg-[#f2ca50] hover:bg-[#e9c349] text-[#3c2f00] rounded text-xs font-semibold transition-all shadow-sm whitespace-nowrap"
              >
              ...
              <a
                href="#vdr"
                onClick={onOpenVdr}
                className="inline-flex items-center justify-center gap-2 px-5 min-h-12 bg-[#161c24] border border-[#f2ca50]/40 hover:border-[#f2ca50] text-[#f2ca50] rounded text-xs font-semibold transition-all whitespace-nowrap"
              >
        ...
        <div className="relative w-full rounded-xl overflow-hidden border border-[#4d4635]/40 group shadow-2xl bg-[#090e17]">
          <div className="relative w-full h-[300px] sm:h-[420px] md:h-[500px] lg:h-[620px]">
            {bannerFailed ? (
              <div
                aria-hidden="true"
                className="w-full h-full bg-[radial-gradient(circle_at_30%_20%,#1d2836_0%,#0d131d_55%,#090e17_100%)]"
              />
            ) : (
              <img
            ...
            <div className="absolute top-6 left-6 lg:top-8 lg:left-8 flex flex-col gap-2">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#090e17]/85 backdrop-blur-md border border-[#f2ca50]/40 rounded font-mono-metric text-[11px] text-[#f2ca50]">
                ...
                <span>SEOUL INVESTMENT COMMITTEE<span className="hidden sm:inline"> • CLOSED SESSION</span></span>
              </div>
            ...
            <div className="absolute bottom-6 left-6 right-6 lg:bottom-8 lg:left-8 lg:right-8 flex flex-col md:flex-row justify-between md:items-end gap-4">
              <div>
                <span className="font-mono-metric text-[11px] text-[#f2ca50] block mb-1">
                  GLOBAL GOVERNANCE DISCRETION
                </span>
                <p className="font-serif-display text-base sm:text-lg lg:text-xl text-[#dee2ef] max-w-xl">
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="p-6 bg-[#161c24] border border-[#4d4635]/30 rounded-xl relative overflow-hidden group hover:border-[#f2ca50]/50 transition-all">
            ...
            <p className="text-xs text-[#d0c5af]/80 mt-2">
              Active Value-Creation 생태계 파트너십 (예시 수치)
            </p>
          </div>
        </div>
