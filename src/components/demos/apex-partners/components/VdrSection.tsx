...
import { ShieldCheck, Fingerprint, History, CheckCircle2, Lock } from 'lucide-react';
...
export default function VdrSection() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    setIsSubmitted(true);
    // Reset form inputs after brief confirmation
    setTimeout(() => {
      setInstitutionName('');
      setTitleDept('');
      setFullName('');
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 flex flex-col gap-6">
            ...
            <p className="text-sm lg:text-base text-[#d0c5af]/90 leading-relaxed">
              APEX PARTNERS의 모든 펀드 운용 내역, 실시간 감사 보고서, 포트폴리오 기업 실사(DD) 자료는 금융 보안 규격(FSC/SEC) 및 256-bit AES 다중 인증 하에 보호됩니다.
            </p>
            ...
            <div className="font-mono-metric text-[11px] text-[#d0c5af]/70 mt-2">
              * 개인 투자자 및 비인가 기관의 접근은 자본시장법에 따라 엄격히 제한됩니다.
            </div>
          </div>
          ...
          <div className="lg:col-span-7 bg-[#161c24] border border-[#4d4635]/40 rounded-xl p-6 lg:p-10 shadow-2xl relative">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              {isSubmitted && (
                <div className="p-3.5 bg-[#00a572]/20 border border-[#4edea3] text-[#4edea3] text-xs rounded text-center flex items-center justify-center gap-2 animate-in fade-in duration-200">
                  <CheckCircle2 className="w-4 h-4 flex-shrink-0" />
                  <span>
                    접수가 성공적으로 완료되었습니다. 전담 IR 파트너가 기관 라이선스 검증 후 2시간 이내에 암호화된 토큰 키를 발송합니다.
                  </span>
                </div>
              )}
            </form>
