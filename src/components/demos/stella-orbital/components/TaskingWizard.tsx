...
import { Satellite, Send, Check } from 'lucide-react';
import { ModalType, TaskingFormData } from '../types';

interface TaskingWizardProps {
  onOpenModal: (type: ModalType) => void;
  onTaskingSubmitted: (data: TaskingFormData) => void;
}

export default function TaskingWizard({ onOpenModal, onTaskingSubmitted }: TaskingWizardProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const data: TaskingFormData = {
      coordinates,
      radius,
      sensor,
      urgency,
      formats,
      orgName: orgName || '국방/공공 위성 관측 사업단',
      corpEmail: corpEmail || 'operations@defense-intel.gov',
      agreedITAR
    };
    onTaskingSubmitted(data);
    onOpenModal('tasking');
  };
          ...
          <form className="space-y-8 pt-8" onSubmit={handleSubmit}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div className="p-4 border border-outline-variant rounded-lg space-y-2.5 bg-surface-container-low/40">
                  ...
                  <label className="flex items-center gap-2.5 text-xs cursor-pointer">
                    ...
                    />
                    <span>일반 스케줄링 (24시간 이내 패스 보장)</span>
                  </label>
                  <label className="flex items-center gap-2.5 text-xs cursor-pointer">
                    <input
            ...
            <div className="p-4 bg-surface-container-low rounded-lg border border-outline-variant space-y-4">
              ...
              <div className="flex items-center gap-2 text-xs text-on-surface-variant">
                ...
                />
                <span>ITAR 수출 통제 규정 및 상업용 지구관측 위성 라이선스 약관에 동의합니다.</span>
              </div>
            </div>
