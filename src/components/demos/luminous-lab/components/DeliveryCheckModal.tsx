  ...
  const handleCheck = (areaName: string) => {
    ...
    if (isInstant) {
      setResult({
        available: true,
        areaName: q,
        message: '오늘 저녁 8시 전 도착 가능 지역입니다! (오늘드림 3시간 내 픽업/배송)',
      });
    } else {
      setResult({
        available: false,
        areaName: q,
        message: '오늘드림 익일 특급 배송 지역입니다. (오늘 주문 시 내일 오전 우체국/CJ 총알도착)',
      });
    }
  };
      ...
      <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-white space-y-5 animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between pb-3 border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-[#141b2b]">오늘드림 가능지역 조회</h3>
          </div>
          <button
            onClick={() => {
              setResult(null);
        ...
        <div className="space-y-4">
          <p className="text-xs text-[#3d4a42]">
            배송받으실 지역(구/동 또는 도로명)을 입력하시면 실시간 오늘드림 배송 가능 여부를 확인해 드립니다.
          </p>
          ...
          <div className="p-3 bg-[#f1f3ff] rounded-xl text-[11px] text-[#3d4a42] space-y-1 leading-relaxed">
            <p className="font-bold text-[#141b2b]">오늘드림 서비스 안내</p>
            <p>• 오후 4시 이전 결제 완료 건: 당일 저녁 8시 전 배송 완료</p>
            <p>• 30,000원 이상 주문 시 오늘드림 무료 배송</p>
          </div>
        </div>
