...
export const TopTicker: React.FC = () => {
  ...
  // Subtle real-time oscillation to simulate live laboratory telemetry
  useEffect(() => {
    const interval = setInterval(() => {
      setBusVoltage(prev => +(804.0 + (Math.random() * 0.4)).toFixed(1));
      setJunctionTemp(prev => +(45.1 + (Math.random() * 0.5)).toFixed(1));
    }, 2800);
    ...
    >
      <div className="flex items-center gap-3 shrink-0">
        <span className="inline-flex items-center gap-1.5 text-[#00e5ff]">
          ...
          <span className="font-bold tracking-wider">LIVE TELEMETRY NODE</span>
        </span>
