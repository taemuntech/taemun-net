...
export default function TechnovaGearPageClient({ isEmbed }: Props) {
  ...
  return (
    <DevicePreviewFrame
      ...
      inquiryUrl="/inquiry?from=technova-gear"
      specs={[
        { label: '반응형 규격', value: '모바일 2열 벤토 랙 & 데스크톱 4열 단일 lg: 브레이크포인트 규격' },
        { label: '정밀 하드웨어 스펙 매트릭스', value: 'GPU(RTX 4090/4080) · CPU · 240Hz OLED · 32GB RAM 다면 대조' },
        { label: '3D 쿨링 챔버 분해도 뷰어', value: '베이퍼 챔버 및 듀얼 블로우 팬 열역학 분해도 인터랙티브 CAD 모달' },
        { label: '섀시 I/O 포트 텔레메트리', value: '썬더볼트4, HDMI 2.1, SDExpress 전후좌우 6면 입출력 단자 매크로 뷰' },
      ]}
    />
  );
}
